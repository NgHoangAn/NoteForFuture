...
## 9.3 Slicing 
```py
# slicing only works with sequence types
...
# selects elements starting at the index start and stopping at the index stop (excluding the element at the index stop)
start <= n < stop
...
# The indices method
## method that returns the equivalent range (start, stop, step) for any slice of a sequence with a specified length
